---
title: "Chain Observability AI Agent and a specialized domain (K8s) agent in Elasticsearch"
...
This guide shows how to keep a parent **Elasticsearch Observability** agent focused on traces, logs, and service graphs for triaging opentelemetry-demo application, running in AWS EKS, while you delegate live **Amazon EKS** inspection to a second agent that only exposes **EKS MCP** tools.
...
### Step 2: Run the EKS MCP bridge and register the connector

Complete [**aws-eks-mcp-setup**](https://github.com/ramp-km/aws-eks-mcp-setup/blob/main/README.md) end to end.
The flow you would be following is : IAM for read-only cluster access, build and push the bridge image, IRSA (IAM roles for service accounts) and **Kubernetes RBAC**, deploy the bridge with a strong **`API_ACCESS_TOKEN`**, then expose the bridge as a service.
In Kibana use **Stack Management → Connectors → MCP** with **`http://<loadbalancer-hostname>:8888/mcp`** and **`Authorization: Bearer <API_ACCESS_TOKEN>`** until **Test** passes.
...
In Agent Builder, create an agent with agent ID : **k8s_troubleshooter** and custom instructions at [**k8s_troubleshooter_agent**](https://github.com/ramp-km/blogs/blob/main/Custom%20K8s%20Troubleshooter/k8s_troubleshooter_agent.md).
Use **Manage MCP → Bulk import** on your connector, then attach **only** those tools to **K8s Troubleshooter**.
...
### Step 4: Duplicate Observability Agent v2 without EKS tools

Duplicate the bundled **Observability** agent (for example **Observability Agent v2**) so it keeps the stock Observability system instructions and tools.
Do **not** attach EKS MCP tools to this copy.
Append short guidance that tells the parent when to call the workflow (after Step 5), including namespace, resource names, and a short list of checks such as ports, selectors, Events, and logs.

**Takeaway:** The parent stays an Observability-first interface for the user.
...
### Step 5: Import the workflow as a callable tool

Import [**k8s_troubleshooter.yaml**](https://github.com/ramprasadkm/elastic-workflows/blob/main/k8s_troubleshooter.yaml) in **Kibana Workflows** and enable it.
The workflow must include the **`troubleshoot`** step that **`POST`**s **`/api/agent_builder/converse`** with **`agent_id: k8s_troubleshooter`** and **`{{ inputs.user_prompt }}`**.
On **Observability Agent v2**, register that workflow as a tool (for example **`custom.k8s_troubleshooter`** or your org’s naming standard; align with Kibana).
If you change **`agent_id`**, update both the YAML and the Agent Builder agent.
...
**Prompt 1:** *Why are error rates increasing for services like checkout, frontend and recommendation?*
Expect Observability tooling to narrow to **product-catalog**, then the workflow to run with a tight **`user_prompt`** (namespace, resource names, suspected Service or port issue, numbered checks).
...
Expect **K8s Troubleshooter** to read **Service**, **Endpoints**, and pods, compare **`targetPort`** to **`containerPort`**, and explain the mismatch with evidence.

**Takeaway:** You see Observability-led triage and cluster-grounded confirmation in one thread.
...
You validated that **Observability** stays the main surface, that **~20** EKS tools live on one specialist, and that **Workflow + converse** records a clear boundary for audits and reviews.
